...
def check_duplicates(data, column_name):
    ...
    if duplicates.any():
        duplicate_values = data[column_name][duplicates].unique()
        return True, duplicate_values
    return False, None
...
def extract_data():
    ...
    try:
        ...
        if key_content_lookup.strip():
            ...
            # 检查填写表B的关键列是否有重复项
            has_duplicates_lookup, duplicate_values_lookup = check_duplicates(lookup_table, key_column_lookup)
            if has_duplicates_lookup:
                messagebox.showwarning("警告", f"填写表B的关键列 '{key_column_lookup}' 有重复项: {duplicate_values_lookup}")

        if not key_column_source:
            key_column_source = source_data.columns[0]

        # 检查源数据表A的关键列是否有重复项
        has_duplicates_source, duplicate_values_source = check_duplicates(source_data, key_column_source)
        if has_duplicates_source:
            messagebox.showwarning("警告", f"源数据表A的关键列 '{key_column_source}' 有重复项: {duplicate_values_source}")
